...
public class TravelingSalesman implements Strategy{
    ...
    /**
     * Determines the next action for the courier based on a set of rules designed to optimize travel and delivery efficiency.
     * The action is chosen based on the courier's current location, the destination of packets, and the distances involved.
     * Steps:
     * 1. If the courier is at the first place and there are packets to pick up, it creates an action to take the packet with the nearest target.
     * 2. If the courier is not at the first place, it checks whether any packets at the current location need to be delivered nearby; if so, it updates the target accordingly.
     * 3. If there are no packets at the courier's location, or all nearby delivery opportunities are exhausted, the courier plans to move towards the location with the highest priority packet or back to the first place.
     * 4. Finally, if the courier is at the location with no pending actions, it stays put by creating an action that targets the current location.
     * @return An Action object indicating the next location to which the courier should move and any packets to take or deposit.
     */
    @Override
